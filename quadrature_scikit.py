...
def OPT_Rand1D(f, prior, iters, actions, initNum, numAdd, saveimgs=False):
    actionEstimate = {}

    for a in actions:
        actionEstimate[a] = init(f(a),prior,initNum)

    for i in range(iters):
        maxa = min(actionEstimate, key=lambda a: actionEstimate[a].mu - 100000.0 * actionEstimate[a].var)
        print maxa
